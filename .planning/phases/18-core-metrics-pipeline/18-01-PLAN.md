...
**Add a test in rust/src/types.rs** (in `#[cfg(test)] mod tests`):
- Test that `CyclomaticConfig::default()` returns the correct values matching Zig defaults:
  ...
  <verify>
    <automated>cd /Users/benvds/code/complexity-guard/rust && cargo test types:: -- --nocapture 2>&1</automated>
    <manual>Verify metrics module compiles with new types and CyclomaticConfig defaults match Zig</manual>
  </verify>
  <done>Metrics module foundation exists with shared types, is_function_node guard, and Cargo dependencies. CyclomaticConfig::default() verified to match Zig defaults. Code compiles cleanly.</done>
</task>
...
- Test `count_decision_points` on `tests/fixtures/typescript/cyclomatic_cases.ts` — use these expected values (from Zig binary output):
  ```
  baseline:                  cyclomatic=1
  simpleConditionals:        cyclomatic=3
  loopWithConditions:        cyclomatic=5
  switchStatement:           cyclomatic=5
  errorHandling:             cyclomatic=3
  ternaryAndLogical:         cyclomatic=3
  nullishCoalescing:         cyclomatic=3
  nestedFunctions:           cyclomatic=2
  arrowFunc:                 cyclomatic=2
  complexLogical:            cyclomatic=5
  DataProcessor.process:     cyclomatic=2
  ```
- Test `count_decision_points` on `tests/fixtures/typescript/complex_nested.ts` — expected:
  ```
  processData:               cyclomatic=11
  ```
  (Only 1 function — nested functions are isolated and NOT reported as top-level.)
...
- Test against `tests/fixtures/typescript/structural_cases.ts` — use these expected values (from Zig binary output):
  ```
  ...
  ```
  File-level: `file_length=41, export_count=4`
