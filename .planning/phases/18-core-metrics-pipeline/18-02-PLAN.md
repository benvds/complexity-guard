...
- Test against `tests/fixtures/typescript/cognitive_cases.ts` — run `./zig-out/bin/complexity-guard --format json tests/fixtures/typescript/cognitive_cases.ts` to get expected values.
...
- Test against `tests/fixtures/typescript/complex_nested.ts` for nested function isolation.
  </action>
  <verify>
...
**Tests** (in `#[cfg(test)] mod tests`):
- Test against `tests/fixtures/typescript/halstead_cases.ts` — run `./zig-out/bin/complexity-guard --format json tests/fixtures/typescript/halstead_cases.ts`.
- **Use float tolerance:** `assert!((actual - expected).abs() < 1e-6)` for all f64 comparisons.
- Test that TypeScript type annotations do NOT inflate operator/operand counts (compare a TS function with types to an equivalent plain JS function).
