...
must_haves:
  truths:
    ...
    - "Documentation (including publication READMEs) reflects parallel pipeline capabilities"
  artifacts:
...
Keep changes minimal — full documentation rewrite is Phase 22.

**Publication READMEs (sync per CLAUDE.md GSD Workflow Rule):**
- Update `publication/npm/README.md` with same Phase 20 parallel pipeline note added to main README.md.
- Update all 5 package READMEs (`publication/npm/packages/darwin-arm64/README.md`, `publication/npm/packages/darwin-x64/README.md`, `publication/npm/packages/linux-arm64/README.md`, `publication/npm/packages/linux-x64/README.md`, `publication/npm/packages/windows-x64/README.md`) — add the same minimal Phase 20 note to keep them in sync with the main README.
  </action>
  <verify>
    <automated>grep -c "Phase 20\|parallel pipeline\|rayon" README.md docs/getting-started.md docs/cli-reference.md docs/examples.md publication/npm/README.md publication/npm/packages/darwin-arm64/README.md publication/npm/packages/darwin-x64/README.md publication/npm/packages/linux-arm64/README.md publication/npm/packages/linux-x64/README.md publication/npm/packages/windows-x64/README.md 2>/dev/null</automated>
    <manual>Verify docs and publication READMEs mention Phase 20 parallel pipeline completion</manual>
  </verify>
  <done>README.md, docs pages, and all publication READMEs (publication/npm/README.md + 5 package READMEs) updated with Phase 20 parallel pipeline notes.</done>
</task>
...
# SC-2: Timing comparison — parallel speedup verification
# Compare --threads 1 vs --threads 4 on the full fixture set.
# NOTE: The test fixture set is small, so the timing difference may be marginal
# or dominated by startup overhead. If the difference is not clearly visible,
# this is expected — rayon's parallel speedup becomes significant on larger
# real-world codebases. The important verification is that both thread counts
# produce identical, correct output; measurable speedup on small fixtures is
# not guaranteed.
cd /Users/benvds/code/complexity-guard/rust && cargo build --release 2>/dev/null
echo "=== threads=1 ===" && time cd /Users/benvds/code/complexity-guard/rust && ./target/release/complexity-guard ../tests/fixtures/ --threads 1 -f json > /dev/null 2>&1
echo "=== threads=4 ===" && time cd /Users/benvds/code/complexity-guard/rust && ./target/release/complexity-guard ../tests/fixtures/ --threads 4 -f json > /dev/null 2>&1

# All existing tests still pass
cd /Users/benvds/code/complexity-guard/rust && cargo test 2>&1 | tail -5
```

All pipeline module tests pass. Release build compiles cleanly with new dependencies. Timing comparison run for SC-2 (parallel speedup may not be measurable on small fixture sets due to startup overhead dominating — see note above).
</verification>
