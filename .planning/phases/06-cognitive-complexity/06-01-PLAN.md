...
must_haves:
  truths:
    ...
    - "Tool does NOT increment for null coalescing ?? in optional chaining context or ?. operator"
...
1. Create `tests/fixtures/typescript/cognitive_cases.ts` with hand-annotated expected cognitive scores (ComplexityGuard rules). Include these cases:
   ...
   - `ternaryNested(x)`: score 3 — ternary (+1 nesting 0, increases nesting) + nested ternary (+1+1 nesting 1) = 3. Wait — ternaries are structural.
   - `classMethod()`: score 2 — class methods start at nesting 0, for-of (+1 nesting 0) + if (+1+1 nesting 1) = 3. Class method itself starts at 0.
